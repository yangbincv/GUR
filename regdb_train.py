...
def main_worker(args):
    ...
    logs_dir_root = osp.join(args.logs_dir+'/'+"regdb_val")
    trial = args.trial
    # global start_epoch, best_mAP
    start_epoch =0
    best_mAP =0
    # logs_dir_root = osp.join(args.logs_dir+'/'+args.dataset+"_ca_pretrain_joint_unsper")
...
#############stage2
        ...
        # cluster_rgb = DBSCAN(eps=rgb_eps, min_samples=4, metric='euclidean', n_jobs=-1)
        # pseudo_labels_rgb = cluster_rgb.fit_predict(features_rgb.cpu())
...
################################ IR2RGB
        ...
        if epoch >= stage_cmass:
            ...
            if epoch  < 100:
                ...
                W       = F.softmax(W, dim=1)
                N = W.size(0)
                topk, indices = torch.topk(W, 10)#20
    ...
    ###########structure smooth soft
            if (epoch >= (stage_cmass+0)):# and (epoch %2 == 0):#5
            ...
            # pseudo_labels_rgb2ir = np.hstack((pseudo_labels_rgb, pseudo_labels_ir_cm))
            # pseudo_labels = pseudo_labels_ir2rgb#np.hstack((pseudo_labels_rgb, pseudo_labels_ir))
        ...
        train_loader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                        128, args.workers, args.num_instances, 100,#100
        ...
        train_loader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                    256, args.workers, args.num_instances, 100,#100
        ...
        if epoch < (stage_cmass+5):
            ...
            # features_ir = F.softmax(features_ir.data, dim=1)



            features_all = torch.cat((features_rgb,features_ir),dim=0)
            # features_all = F.normalize(features_all) 
            ...
            print('==> Statistics for ir  epoch {}: {} clusters,cluster data {}'.format(epoch, num_cluster_ir,len(pseudo_labeled_dataset_ir)))
            train_loader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                        256, args.workers, args.num_instances, 100,
                                        trainset=pseudo_labeled_dataset_ir, no_cam=args.no_cam,train_transformer=transform_thermal)
